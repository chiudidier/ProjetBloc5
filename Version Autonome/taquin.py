...
def ids(etatdepart, path):
    ...
    Fonction qui teste les états suivants en augmentant progressivement la profondeur maximale de recherche
    ...
    Sorties : aucune, mais la modification de la variable path permet de récupérer le chemin entre l'état de départ et l'état solution
...
def nbcoup(etat):
    ...
    Fonction qui calcule la distance de Manhattan pour toutes les tuiles du taquin (voir heuristique)
    ...
    Sorties : 
        ...
        - la somme des distances de Manhattan
...
def dlsplus(profmax, etatdepart, profdepart, path):
    ...
    # vérification si l'état testé est gagnant ou non
...
# Fonction qui creer une seed aléatoire à pour générer un taquin solvable. La vérification se fait par contrôle du désordre

def random_init(statetaquin):
